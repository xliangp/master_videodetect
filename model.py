...
class VisTransformLayer(nn.Module):
    def __init__(self,input_size):
        ...
        self.pool_input_size = 3072
        self.input_size = input_size
        self.fc6 = nn.Linear(self.pool_input_size, 1024) 
        self.relu6 = nn.ReLU()
        self.dpt6 = nn.Dropout(p=0.5)
        self.fc7 = nn.Linear(1024, 1024)
        self.relu7 = nn.ReLU()
        self.dpt7 = nn.Dropout(p=0.5)
        self.fc8 = nn.Linear(1024, self.input_size)
    def forward(self, inputs):
...
class SST_AD_PreRel(nn.Module): # Note: basic pre-release version... will be updating code for actual release. 
    ...
    def __init__(self, num_proposals=16, num_classes=21, num_rnn_layers=1, h_width=512, input_size=500, dropout=0):
        super().__init__()
        self.FeatTrans=VisTransformLayer(input_size)
    ...
    def forward(self, inputs):
        inputsT=self.FeatTrans(inputs)
        (proposals_out, h_prop), (class_out, h_cls) = \
                (self.SST_Prop.forward(inputsT), self.SST_SegAct.forward(inputsT))
        ...
        # output: (N*L*K, C), proposals_out: (N,L,K)
        #class_out: (N, L, C), final_joint_cls_scores:(N ,L, K, C)
